...
```q2
This package is inspired by the "Unit testing" technique, where additional code ('test code'), is written to validate functional units of the source code against a set of rules ('test cases.')

These tests are then used to detect errors and test functionality as changes to the code base made. It does this by ensuring the desired behaviour is exhibited and it not it produce results that violate the rules defined in test cases. 

We apply the same concepts to 'data,' where we define 'data test cases' and validate the dataset using these rules.
...
In reproducible research the data cleaning procedure should be automated such that it self documents what was done and can be reproduced. This process normally involes four general steps; converting the data types; filtering data; transforming the data into the correct shape and map it onto new values and putting it in a standardized format such as tidy format. This process is complex and is error prone. Validation of the steps along with characteristics needs to be performed.

Data Scientists can use this tool to add a new 'data validation' step after the 'data cleaning' step. In this 'data validation' step, we look at the data profile and define rules ('assumptions') about the data.

Next time that we process a new 'version' of the dataset, we can verify that these assumptions about the data are still valid. 

Here are some examples of verifications:  
1. factor 'state' has 50 levels, and each level has observations.
...
```q4
The main function will be defined as test_data(description, code_block). The data test cases 
will be defined using functions of the form 'test_data(description, code_block)' 
in a similar way of the 'test_that' function from the testthat package.

There will be a family of functions starting with 'expect_' that will test the data. 
Here are some of examples: 
  'expect_length', 'expect_mean', 'expect_lte', 'expect_gt', etc.

We are wrapping the package 'testthat' in our 'testdata' package. The testthat package is not 
vectorized but testdata will be. For example:
test_that( "sum should be 55", { 
   expect_equal(sum(1:10), 55) 
} )

A vectorized version would be
test_data( "equal paired values", { 
   expect_equal( c(1, 2, 3, 4, 5, 7, 8, 9, 10), 1:10)
})
```

...
expect_equal <- function(object, expected, ..., info = NULL, label = NULL,
                         expected.label = NULL) {
   # TODO:
   # For some reason the tolerance was no getting set at a lower level
   # when calling the testthat::expect_equal at the level of all.equal
   # there is a default value that was not being set
   dots <- list(...)
   if ( is.null(dots$tolerance) ) {
       dots$tolerance = sqrt(.Machine$double.eps)
   }
   testthat::expect_equal(object, expected, unlist(dots), info, label, expected.label)
}
...
expect_equivalent <- function(object, expected, ..., info = NULL, label = NULL,
                              expected.label = NULL) {
   testthat::expect_equivalent(object, expected, ..., info, label, expected.label = NULL)
}
